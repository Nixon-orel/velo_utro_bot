...
После обновления кода всегда применяйте миграции до запуска приложения. Миграция `007` добавляет `events.published_at`, которое заполняется только после успешной отправки сообщения в Telegram. Миграции `008`–`010` добавляют постоянную очередь Telegram-доставок, возрастающую версию погодного расписания события и отметку завершённой сверки доставки. Миграция `011` разрешает общесистемным сообщениям очереди, таким как ежедневный анонс, не ссылаться на конкретное событие.
...
│       ├── 010_add_finalized_at_to_notification_deliveries.rb # Сверка завершённых доставок
│       └── 011_make_event_optional_on_notification_deliveries.rb # Общесистемные доставки
...
- **Надёжная доставка погодных уведомлений и ежедневных анонсов**: каждое сообщение или редактирование публикации хранится отдельно в PostgreSQL, повторяется с задержкой после временной ошибки Telegram и не отправляется заново после подтверждённого успеха
...
- Автоматический ежедневный анонс в канале содержит только опубликованные события, которые начнутся в ближайшие 24 часа; повторная отправка завершённой подборки блокируется на 20 часов
- Заголовок и каждое событие анонса сохраняются как отдельные записи очереди. После частичного сбоя или перезапуска продолжаются только незавершённые сообщения; после пяти ошибок одной записи остальные элементы подборки всё равно обрабатываются, но анонс не отмечается успешным
...
После пяти неуспешных попыток доставка получает статус `failed` и требует внимания администратора. `/scheduler_status` показывает ожидающие и окончательно не доставленные части анонсов, а `/weather_status` — состояние погодных доставок. Уже подтверждённые сообщения не отправляются повторно при обычной обработке. Однако если процесс аварийно завершится после принятия сообщения Telegram, но до записи успеха в PostgreSQL, повторная отправка возможна: Telegram API не предоставляет ключ, который полностью исключил бы это короткое окно.
...
RSpec прекращает работу, если окружение отличается от `test` или имя базы не заканчивается на `_test`. Реальные HTTP-запросы во время тестов запрещены через WebMock. Интеграционный набор проверяет защиту очереди от дублей при конкурентной работе, повторные попытки, передачу просроченной блокировки новому обработчику, частичную доставку погодных сообщений и ежедневных анонсов, восстановление после перезапуска, смену версии события и завершение потерянной сверки. Откат связанных миграций разрешён только при сохранении целостности очереди; защитные проверки тестируются отдельно. Отчёт покрытия создаётся локально в `coverage/`.

Безопасные smoke-проверки загрузки приложения, запуска Telegram runner без токена и Sinatra endpoints входят в автоматический набор. Они не подключаются к реальным Telegram и WeatherAPI. Живую проверку с тестовым ботом и тестовым каналом выполняют отдельно только с выделенными тестовыми секретами.
